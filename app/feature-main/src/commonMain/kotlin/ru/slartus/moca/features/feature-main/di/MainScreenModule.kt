...
val mainScreenModule = DI.Module("mainScreenModule") {
    ...
    bindSingleton(tag = "animation.movies") { ProductsViewModel<Movie>(instance("animation.movies"), instance()) }
    bindSingleton(tag = "animation.series") { ProductsViewModel<Series>(instance("animation.series"), instance()) }
}
